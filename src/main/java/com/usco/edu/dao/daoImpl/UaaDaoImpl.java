...
import com.usco.edu.resultSetExtractor.DependenciaSetExtractor;
...
public class UaaDaoImpl implements IUaaDao {
	...
	public List<Uaa> findByTipoUaaPag(String userdb, int tipoUaa, int page, int records) {
		...
		String sql = "DECLARE @RowsPerPage INT = ?, @PageNumber INT = ? "
				+ "SELECT uaa_codigo ,uaa_estado , uaa_nombre , uaa_nombre_corto, sed_codigo ,sed_nombre " + "FROM ( "
	...
	public void newUaaDpto(String userdb, Uaa uaa) {
		...
		String sql = "INSERT INTO uaa (uaa_nombre,uaa_dependencia,uaa_nombre_corto,uaa_email, "
				+ "sed_codigo,uaa_centro_costos,uaa_estado,uat_codigo )" + "VALUES ( ?, ?, ?, ?, ?, ?, ?, ?) ";
		try {
	...
	@Override
	public List<Dependencia> findSedesRestaurante(String userdb) {
		String sql = "SELECT * FROM dbo.uaa u WHERE u.uaa_nombre like '%restaurante%'";
		List<Dependencia> uaas = null;
		try {
			uaas = jdbcTemplate.query(sql, new DependenciaSetExtractor());
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
